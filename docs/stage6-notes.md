...
| | 向量存储 / 检索（Milvus / Qdrant / Chroma / FAISS） | Elasticsearch / OpenSearch |
|---|---|---|
...
embedding 模型只负责把文本变成向量；向量存储负责保存并搜索这些向量。典型记录可以理解为：

```text
chunk_id: 42
vector:  [0.021, -0.043, 0.118, ...]
text:    "退款会在 3～5 个工作日到账"
metadata: {tenant: "A", source: "退款政策.pdf", page: 7}
```

查询时，问题先被同一个 embedding 模型转成向量，检索器再找距离最近的 `top-k` 条记录，
返回对应原文。数据量小时可以逐条精确比较；数据量大时通常建立 ANN（近似最近邻）索引，
用少量召回率换取显著的速度和内存收益。常见索引有 Flat、IVF、HNSW，但它们只是不同的
查找策略，不会让 embedding 本身更懂语义。

**FAISS 严格来说不是数据库，而是 Meta 开源的向量检索库。** 它嵌入应用进程，负责建立
索引和执行最近邻搜索，也能把索引保存到文件；但原文、metadata、权限过滤、增删改、并发
服务、备份和集群通常要由应用自己管理。它适合实验、单机原型和离线检索。

**Milvus 是向量数据库。** 它把向量索引包装成独立服务，并提供 collection/schema、持久化、
增删改查、metadata 过滤、分区、备份及分布式扩缩容等数据库能力，适合数据量大、并发高或
需要独立运维的生产系统。

| 工具 | 本质 | 数据与生命周期 | 适合场景 |
|---|---|---|---|
| `InMemoryVectorStore` | LangChain 内存实现 | 进程退出即丢 | 本仓库教学 demo |
| FAISS | 向量索引/算法库 | 应用自行保存索引并管理原文 | 单机原型、离线任务 |
| Chroma | 轻量向量数据库 | 可本地持久化，开箱简单 | 小型本地 RAG |
| pgvector | PostgreSQL 扩展 | 复用 PostgreSQL 的事务、权限和备份 | 已有 PostgreSQL 的系统 |
| Milvus / Qdrant | 独立向量数据库 | 服务化、持久化、过滤和扩容 | 大规模生产检索 |

它们都**不能替代 embedding 模型**：模型决定向量是否表达语义，FAISS/Milvus 只负责把
已经得到的向量存好、快速找近邻。选型也不必一步到位：本 demo 用内存实现已经足够；已有
PostgreSQL 就优先考虑 pgvector，已有 Elasticsearch 可直接使用其 `dense_vector`/kNN，只有
规模和运维需求确实超出已有系统时，再引入 Milvus 这类独立服务。
